...
class GATEmbedder():
    ...
    def gat_learn(self, stellargraph, df_onehot):
        ...
        generator = FullBatchNodeGenerator(stellargraph, method="gat")
        train_gen = generator.flow(train_targets.index, train_targets.to_numpy())
        val_gen = generator.flow(test_targets.index, test_targets.to_numpy())
        all_gen = generator.flow(df_onehot.index)

        gat = GAT(layer_sizes=[8, df_onehot.shape[1]],
                  activations=["elu", "softmax"],
                  attn_heads=self.param.DIMENSIONS/8,
                  generator=generator,
        ...
        model.compile(optimizer=tf.keras.optimizers.Adam(learning_rate=self.param.LEARNING_RATE_GAT),
        ...
        mc_callback = tf.keras.callbacks.ModelCheckpoint("logs/best_model.h5",
                                                         monitor="val_acc",
                                                         save_best_only=True,
