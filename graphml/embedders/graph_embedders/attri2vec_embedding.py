...
class Attri2VecEmbedder():
    ...
    def create_stellargraph(self, graph):
        ...
        df_onehot = pd.DataFrame(vec_onehot, index=df.index, columns=enc.categories_)
    ...
    def atrri2vec_learn(self, stellargraph):
        ...
        unsupervised_samples = UnsupervisedSampler(stellargraph,
                                                   nodes=nodes,
                                                   length=self.param.WALK_LENGTH,
                                                   number_of_walks=self.param.NUM_WALKS
                                                   )
        generator = Attri2VecLinkGenerator(stellargraph, self.param.BATCH_SIZE)
        attri2vec = Attri2Vec(layer_sizes=[self.param.DIMENSIONS],
                              generator=generator,
                              bias=False,
                              normalize=None
                              )
        ...
        node_gen = Attri2VecNodeGenerator(stellargraph, self.param.BATCH_SIZE).flow(nodes)
        node_embeddings = embedding_model.predict(node_gen, workers=self.param.WORKERS, verbose=0)
