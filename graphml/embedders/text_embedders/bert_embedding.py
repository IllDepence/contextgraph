...
def _get_embeddings(text, tokenizer, model, param):
    ...
    '''
    Internal use only. Outputs the average of last four layers of
    BERT model as embedding for the text.
    '''
    ...
    with torch.no_grad():
        ...
        # variable 'hidden_state' has 13 layer,
        # each has torch.Size([1, 512, 768])
...
def create_bert_embedding_for_ml(
        df_graph_embedding,
        param,
        tokenizer,
        model
    ...
    df_graph_embedding: embedding dataframe of graphs, used for
                bert embedding results to find respective target
                nodes, so the two dataframe can be concatenated
                without error
    param: object of class Parameters
    tokenizer: Bert tokenizer, initialized in ../classification.py
    mdoel: bert model (bert-base-uncased), initialized in
                ../classification.py
    '''
    ...
    df_bert = pd.DataFrame(
        df["description"].apply(
            lambda x: _get_embeddings(x, tokenizer, model, param)
        )
    )
    df_bert = pd.DataFrame(
        df_bert["description"].tolist(),
        index=df_bert.index
    )
