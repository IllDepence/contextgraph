...
def evaluate(results_cv, test_data, test_label, display=False):
    ...
    for clf in results_cv:
        ...
        conf_matrix = pd.DataFrame(confusion_matrix(test_label, prediction),
        ...
        '''
        Claimer: the four usual performance measures displayed are little bit different from calculated 
        results from confusion matrix, that is because the the Confusion Matrix is constructed based on a
        "Voting" methods from all estimators, while performance measures displayed here are calculated by
        the average of performance measures of all estimators (details see code above) 
        '''
        if display:
...
def get_baseline_performance(dataseries, method, univocal_label="pos", num_average=10, display=True):
    if method=="random":
        accuracies, precisions, recalls, f1s = [], [], [], []
        for i in range(num_average):
            random_labels = stats.bernoulli.rvs(0.5, size=dataseries.shape[0])
            accuracies.append(accuracy_score(dataseries.values, random_labels))
            precisions.append(precision_score(dataseries.values, random_labels))
            recalls.append(recall_score(dataseries.values, random_labels))
            f1s.append(f1_score(dataseries.values, random_labels))
        acc = mean(accuracies)
        pre = mean(precisions)
        re = mean(recalls)
        f1 = mean(f1s)
        ...
        raise Exception(f'method {method} for performing Baseline prediction not supported, '
                        f'please use one from "random" or "univocal"')
    if display==True:
        print(f'Performance of Baseline prediction using {method} method on test data:')
